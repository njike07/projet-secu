...
try {
    ...
    if (!$id_etudiant) {
        die("Vous devez être connecté pour modifier vos informations.");
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql = "UPDATE compte SET 
                ...
                dernier_diplome_obtenue = :dernier_diplome_obtenue,
                etablisement_precedent = :etablisement_precedent,
                formation_demande = :formation_demande,
                specialisation = :specialisation,
                nom_contact_urgence = :nom_contact_urgence,
                relation_etudiants = :relation_etudiants,
        ...
        $stmt->bindParam(':id_etudiant', $id_etudiant); // Ajout de l'identifiant pour cibler l'étudiant spécifique

        // Exécuter la requête
        if ($stmt->execute()) {
            echo "<div style='color: green; font-weight: bold;'>Vos informations ont été mises à jour avec succès.</div>";
        } else {
            echo "<div style='color: red; font-weight: bold;'>Erreur lors de la mise à jour. Veuillez réessayer plus tard.</div>";
        }
    }
